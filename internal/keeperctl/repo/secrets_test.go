...
func doGetSecret(
	t *testing.T,
	mockRV *goph.GetResponse,
	mockErr error,
) (*goph.Secret, []byte, error) {
	t.Helper()

	id := uuid.NewV4()
	req := &goph.GetRequest{Id: id.String()}
...
func doUpdateSecret(
	...
	m.On(
		...
		Return(&goph.UpdateResponse{}, clientErr)
...
func doDeleteSecret(t *testing.T, mockErr error) error {
	...
	id := uuid.NewV4()
	req := &goph.DeleteRequest{Id: id.String()}
	...
	m.On(
		...
		Return(&goph.DeleteResponse{}, mockErr)
